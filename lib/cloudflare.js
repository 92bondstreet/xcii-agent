 ...
 */
const flare = (product, body) => {
  const urlObject = url.parse(product);
  const host = urlObject.hostname;
  const origin = `${urlObject.protocol}//${host}`;
  ...
  const solution = `${origin}/cdn-cgi/l/chk_jschl?jschl_vc=${answer.jschl_vc}&pass=${answer.pass}&jschl_answer=${answer.jschl_answer}`;
 ...
 * @return {Promise}
 */
module.exports.solvecf = (product, body) => {
  return new Promise((resolve, reject) => {
    const answer = flare(product, body);

    setTimeout(async () => {
      try {
        const heads = Object.assign({}, HEADERS, {
          'Host': url.parse(product).hostname,
          'Referer': product
        });

        const response = await request.agent().set(heads).get(answer);
